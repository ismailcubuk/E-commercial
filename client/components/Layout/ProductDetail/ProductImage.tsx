...
export default function ProductImage({ product, title, variant }: any,) {
    ...
    function renderFeature(featureName: string, label: string, unit: string) {
        ...
        const handleLinkClick = (memory:string) => {
            router.push(`/products/${product.category}/${title}-${variant}-${memory}%20GB`);
        }
